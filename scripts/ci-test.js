...
function run(command, extraEnv = {}, stepLabel = null) {
  ...
  if (thrownError) {
    if (isVitest) {
      // Test failure — record and continue; the report will show what failed.
      _hasTestFailures = true;
      console.error(`  ⚠️  Some tests failed — continuing script...\n`);
    } else {
      // Infrastructure failure (signing script, etc.) — must stop immediately.
      throw thrownError;
    }
  }
}
...
function generateReport(overallSuccess, startTime = null) {
  try {
    const now = new Date();
    ...
    lines.push(`| ⏱ Duration | ${elapsedStr} |`);
    lines.push(``);
    lines.push(`---`);
    lines.push(``);
    ...
    lines.push(`---`);
    lines.push(``);
    lines.push(`*Generated by \`scripts/ci-test.js\` · ${dateStr} at ${displayTime} · Suite: \`${TEST_SUITE}\` · Duration: ${elapsedStr}*`);
...
try {
  ...
  const phase1Excludes = [
    ...
    "account-data",      // run separately so transaction-data can follow immediately after
    "transaction-data",  // run right after account-data (needs GET_TRANSACTIONS_ACCOUNT_ID)
    "init-passkey",
    "transaction-passkey",
    "user-invitation",
    "init-otp",
  ...
  // account-data runs after catch-all so transaction-data can follow immediately.
  ...
  if (isPasskeyEnabled && canSign) {
    ...
    // Wait for deposit to settle before querying balances
    console.log("\n  ⏳ Waiting 15s for deposit to settle before querying balances...");
    await new Promise((resolve) => setTimeout(resolve, 15_000));

    // Account balances (post-deposit-submit) — uses CI_PASSKEY_ACCOUNT_ID
    console.log("\n  ── Account balances (post-deposit-submit) ──");
    const balancesFilter = TEST_SUITE === "sdk" ? "getAccountBalances" : "get-account-balances";
    run(
      `npx vitest run ${testFile("account-data")} -t "${balancesFilter}"`,
      { CI: "true" },
  ...
  if (inviteEnabled && isPasskeyEnabled && canSign) {
    ...
    try {
      ...
      if (otpCode) {
        ...
        run(
          `npx vitest run ${testFile("otp-authentication")} -t "should create virtual authenticator"`,
          {
            ENABLE_OTP_INIT_AUTH_TESTS: "false",
            ENABLE_OTP_AUTHENTICATE_TESTS: "false",
  ...
  generateReport(!_hasTestFailures, scriptStartTime);

  const totalElapsed = Date.now() - scriptStartTime;
  const totalMin = Math.floor(totalElapsed / 60_000);
  const totalSec = Math.floor((totalElapsed % 60_000) / 1000);
  console.log(`\n${DIVIDER}`);
  if (_hasTestFailures) {
    console.log("  ❌ CI tests finished with failures (see report)");
  } else {
    console.log("  ✅ All CI tests completed successfully");
  }
  console.log(`  ⏱  Total duration: ${totalMin}m ${totalSec}s`);
  console.log(`${DIVIDER}\n`);
  if (_hasTestFailures) process.exit(1);
} catch (err) {
